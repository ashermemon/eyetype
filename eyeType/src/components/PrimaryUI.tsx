...
import type { ContextBarHandle } from "./ContextBar";
...
type Props = {
  ...
  shouldStartMicrophone?: boolean;
};

export interface PrimaryUIHandle {
  startMicrophone: () => void;
}

const PrimaryUI = forwardRef<PrimaryUIHandle, Props>(
  (
    {
      activeKey,
      gazeData,
    ...
    useImperativeHandle(ref, () => ({
      startMicrophone: () => {
        contextBarRef.current?.startMicrophone();
      },
    }));

    // Start microphone when flag is true and ref is ready
    useEffect(() => {
      if (shouldStartMicrophone && contextBarRef.current) {
        contextBarRef.current.startMicrophone();
      }
    ...
    useEffect(() => {
      ...
      const timer = setTimeout(async () => {
        ...
        const addUniquePrediction = (prediction: string) => {
          if (controller.signal.aborted) return;
          setPredictions((prev) =>
            prev.includes(prediction) ? prev : [...prev, prediction],
          );
        };

        try {
          const results = await fetchTop3Expansions(
            contextValue,
            aiInput,
            addUniquePrediction,
            controller.signal,
      ...
      if (spellMode && spellSentence && isWordChanged) {
        ...
        if (words[focusedIndex] !== undefined) {
          ...
          if (newSentence !== typedString) {
            ...
            setPredictions(
              predictions.map((p, i) =>
                i === currentPredictionNum ? newSentence : p,
              ),
            );
          }
        }
    ...
    const handleSelectPrediction = (text: string, predictionNum: number) => {
      //setTypedString(text);
      setSpellMode(true);
      setSpellSentence(text.toUpperCase());
      ...
      <>
        ...
        {zoomedSection === null && (
          <>
                  ...
                  {studyMode === "pro" || studyMode === null ? (
                    <>
                      ...
                      {predictions.length > 0 ? (
                        ...
                        <>
                          <PredictedSentence
                            sentenceText={stripPunctuationSpaces(
                              "Welcome! Start typing to see predictions",
                            )}
                          />
                          <PredictedSentence
                            sentenceText={stripPunctuationSpaces(
                              "Abbreviations will be expanded here.",
                            )}
                          />
                          <PredictedSentence
                            sentenceText={stripPunctuationSpaces(
                              "Click on a predicted sentence to edit/spell words",
                            )}
                          />
                        </>
                      ) : null}
            ...
            <div
              className={`top-bar-container ${studyMode === "basic" ? "basic-mode" : ""}`}
            >
              {studyMode !== "basic" &&
                (spellMode ? (
                  <TopBarButton
...
PrimaryUI.displayName = "PrimaryUI";
